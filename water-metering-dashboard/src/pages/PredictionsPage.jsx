...
import {
  ...
  CircularProgress, Chip, Fade, Tooltip, Button
} from '@mui/material';
import { Brain, Send, Bot, ChevronRight, Sparkles, Trash2, RefreshCw } from 'lucide-react';
...
export default function PredictionsPage() {
    ...
    try {
      ...
      if (!res.data || res.data.length === 0) {
        setMessages([{
          ...
          texte: t(
            'ai_greeting',
            "Bonjour ! Je suis WaterBot, l'assistant Wicmic. Posez-moi une question sur vos compteurs, consommations ou anomalies."
          )
        }]);
      } else {
      ...
      setMessages([{
        expediteur: 'ai',
        texte: "Bonjour ! Je suis WaterBot. Comment puis-je vous aider ?"
      }]);
    }
  };
  ...
  const handleClearChat = async () => {
    try {
      await axios.delete('http://127.0.0.1:8000/chat/messages');
      setMessages([{
        expediteur: 'ai',
        texte: "Historique effacé. Comment puis-je vous aider ?"
      }]);
    } catch (error) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:8000/chat/envoyer', {
        message: userMsg
      });
      ...
      const aiText =
        res.data?.ai_message ||
        res.data?.message ||
        t('ai_error', "Désolé, je n'ai pas pu générer de réponse.");
      ...
      const errMsg =
        ...
        t('ai_error', 'Désolé, une erreur de communication est survenue.');
      setMessages(prev => [...prev, { expediteur: 'ai', texte: errMsg }]);
    } finally {
      setIsTyping(false);
    }
  };
        ...
        <Grid item xs={12} md={5} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Box sx={{
                p: 1.5, borderRadius: '12px', bgcolor: '#e0f2fe',
                display: 'flex', alignItems: 'center', justifyContent: 'center'
              }}>
                  ...
                  <Fade in={true} timeout={400 + index * 150} key={insight.id}>
                    ...
                    >
                      ...
                      <Typography variant="h6" sx={{
                        fontWeight: 700, color: '#1e293b', mb: 1, fontSize: '1rem'
                      }}>
                        {insight.titre || '—'}
                      </Typography>

                      <Typography variant="body2" sx={{
                        color: '#475569', mb: 2, lineHeight: 1.6, fontSize: '0.9rem'
                      }}>
                        {insight.description || '—'}
                      </Typography>
                      ...
                      {insight.conseil_ia && (
                        <Box sx={{
                          bgcolor: '#f8fafc', p: 1.5, borderRadius: '8px',
        ...
        <Grid item xs={12} md={7} sx={{ height: '100%' }}>
          <Paper elevation={0} sx={{
            ...
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box sx={{ position: 'relative' }}>
                  ...
                  <Box sx={{
                    ...
                    width: 12, height: 12, bgcolor: '#22c55e',
              ...
              <Tooltip title="Effacer l'historique de conversation">
                <IconButton
                  ...
                  size="small"
                  sx={{
                    bgcolor: '#fef2f2', color: '#ef4444', borderRadius: '8px',
            ...
            <Box sx={{
                ...
                <Fade in={true} key={index}>
                  <Box sx={{
                    ...
                    <Box sx={{
                      ...
                      boxShadow: msg.expediteur === 'ai'
                        ? '0 2px 8px rgb(0 0 0 / 0.07)'
                        : '0 4px 10px rgb(2 132 199 / 0.25)',
              ...
              {isTyping && (
                <Box sx={{
                  ...
                  alignItems: 'flex-end', gap: 1.5, mb: 2.5
                }}>
                  <Avatar sx={{ width: 32, height: 32, bgcolor: '#e2e8f0', flexShrink: 0 }}>
                    <Bot size={18} color="#475569" />
                  </Avatar>
            ...
            <Box
              ...
              sx={{ p: 2.5, bgcolor: '#ffffff' }}
            >
              <TextField
                fullWidth
                placeholder={t(
                  'ask_question_placeholder',
                  "Ex: 'Quel est l'état du compteur MTR-001 ?' ou 'Y a-t-il des anomalies ?'"
                )}
                value={inputMessage}
                ...
                InputProps={{
                  ...
                  endAdornment: (
                    <Box sx={{
                      bgcolor: inputMessage.trim() && !isTyping ? '#0284c7' : '#e2e8f0',
                      borderRadius: '50%',
              ...
              <Typography variant="caption" sx={{
                color: '#94a3b8', mt: 1, display: 'block', textAlign: 'center'
              }}>
